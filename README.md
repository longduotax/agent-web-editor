...
one-use launch URL printed by the server. `--port` overrides `PI_WEB_PORT`; the
backend default is `3001`. `PI_WEB_DEV_PORT` selects the Vite port and defaults
to `5173`. Copy `.env.example` to `.env.local` to keep these development values
in one ignored repository-local file. Application metadata defaults to
