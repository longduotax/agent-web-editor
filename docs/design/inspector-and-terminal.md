...
- The requested root as an entry: `path=.git` and `path=.git/refs` refused in both modes, `?path=.git/config` refused on the single-file read, an ignored directory refused as a listing root and accepted under the opt-in, an ignored-but-tracked directory accepted, and containment unchanged.
