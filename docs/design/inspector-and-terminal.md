...
- **The path a request names is subject to the rules its entries are.** The filter used to be applied only to entries met while walking, so a directory that has no row to click was still reachable by asking for it: `path=.git` listed the repository's machinery, `path=.git/config` returned the config including the remote URL, and `path=frontend/node_modules` returned 390 entries with the opt-in off. `.git` is now refused at the resolve step on every file route, in both modes, and an ignored directory is refused as a listing root unless `showIgnored` is set — except when it holds something tracked, which is the case in which Git shows it too. A **single-file read** applies the `.git` refusal and not the ignore filter: a File tab is opened from a tree that may legitimately be showing ignored paths, it is durable and carries no ignore mode of its own, and a check every real caller would have to bypass is not a boundary. The ignore rules govern what a listing offers, which is what the product requires of them.
- A path that does not exist, one that exists but is the wrong kind, and one that cannot be read are each a typed failure — `path_not_found` (404), `path_not_directory` / `file_not_regular` (400), `path_unreadable` (403) — rather than an internal error. A persisted expansion pointing at a deleted directory is the ordinary case, and the client renders it as that row's own error state.
...
- **A path the Git index tracks is never hidden by an ignore rule**, because Git does not hide one either: a file committed before the rule that now matches it, or added with `git add --force`, is in the repository, and a tree that says otherwise is telling the user their own committed file does not exist. The pattern matcher cannot answer this — the answer is in the index, not in the patterns — so the tracked set is read once per working tree with a single bounded `git ls-files -z --cached` under the Git process policy below, and cached against the identity of `.git/index`, which is the file that command reads and the file every `add`, `commit`, `rm`, and `checkout` writes. Where the index cannot be stamped, the cache lives five seconds; it is re-read after a minute regardless, holds at most four working trees and at most 50,000 paths each, and every failure — no Git, no repository, a non-zero exit, a timeout, truncated output, or more paths than the bound allows — degrades to the pattern matcher alone, which is exactly the behaviour that shipped without it. This is not the rejected `git check-ignore`: that is a per-path oracle consulted on the hot path for every entry of every listing, and this is one bounded listing per working tree, consulted from memory thereafter, answering a question the matcher cannot answer at all. Inside a directory that is excluded and reached only because something tracked lives under it, **only** tracked paths are shown — Git's own rule that a path under an excluded directory cannot be re-included.
...
- Diffs are fetched for one selected status path at a time. Staged and unstaged sections are requested separately with fixed arguments and returned as labeled bounded unified text. Untracked files are previewed by comparing the file against `/dev/null` with `git diff --no-index --no-ext-diff -- /dev/null <path>` — an argument array under the same process policy, never shell redirection — so the browser receives Git's own unified output for a file Git does not track, bounded like every other diff. **Corrected 2026-08-23:** this bullet previously described an application-generated preview built from bounded file content, which is not what the server does and never was; the shape and the bounds are the same, the producer is not. Every diff call carries `--no-ext-diff`, so a user's `diff.external` cannot decide what the browser renders.
...
- The terminal WebSocket checks exact Host and Origin headers during upgrade, requires no credential, and parses versioned `attach`, `create`, `input`, `resize`, `restart`, and `terminate` frames. Input is capped at 64 KiB per frame; dimensions are integers within 2–500 columns and 2–200 rows. `restart` carries the same optional `cwd` as `attach` and `create`, because a replacement has no directory of its own to inherit and the client's record is the only value that is not `null` on a platform that cannot observe one.
- **A refusal the client must act on differently carries a code.** The `error` frame's optional `code` distinguishes `terminal_limit_reached`, `terminal_gone`, and `terminal_cwd_invalid`; every other refusal stays the single opaque message it has always been. The message is the server's own prose in all cases — never text from the error, which could carry a path or a command line.
...
- The requested root as an entry: `path=.git` and `path=.git/refs` refused in both modes, `?path=.git/config` refused on the single-file read, an ignored directory refused as a listing root and accepted under the opt-in, an ignored-but-tracked directory accepted, a non-existent path as `path_not_found`, a file listed as a directory and a directory read as a file each as their own typed rejection, and containment unchanged.
- Tracked paths against a real Git index: a tracked file an ignore rule matches listed and found by search, an untracked sibling still hidden, only the tracked children of an excluded directory shown, a directory with nothing tracked under it still hidden, the listing agreeing with `git ls-files`, a non-Git project unchanged, a cached index reused and re-read after the index changes, and a path containing a newline.
...
- Diffs against a real repository: a staged and an unstaged change of one file returned as two labeled sections, an untracked file previewed against `/dev/null`, a binary file reported as binary rather than as text, a path that has stopped being changed rejected as `git_path_not_changed`, and a directory that is not a repository represented explicitly.
