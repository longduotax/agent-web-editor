...
- partial unique index on `project_id` while state is `running`
...
- Run acceptance writes the receipt/run and acquires the partial unique project lease atomically.
...
- Empty-to-v1 migration, repeated startup, pending-migration backup, rollback, interrupted migration, and newer-version refusal.
...
- Thread/project ownership, concurrent one-running-run constraint, receipt retry/conflict, and completion/viewed transactions.
